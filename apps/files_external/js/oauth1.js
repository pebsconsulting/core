$(document).ready(function() {
	...
	$('#externalStorage').on('click', '[name="oauth1_grant"]', function(event) {
		...
		if (app_key != '' && app_secret != '') {
			...
			$.post(OC.filePath('files_external', 'ajax', 'oauth1.php'), { step: 1, app_key: app_key, app_secret: app_secret, callback: location.protocol + '//' + location.host + OC.generateUrl(location.pathname) + '?sectionid=storage' }, function(result) {
				if (result && result.status == 'success') {
